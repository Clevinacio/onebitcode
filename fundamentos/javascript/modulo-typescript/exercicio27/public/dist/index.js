...
function sendInMission(spaceship) {
    if (spaceship.inMission) {
        ...
        alert(`A nave ${spaceship.name} não possui tripulantes para iniciar a missão!`);
    }
    else {
...
function firstMenuOption() {
    ...
    const confirmation = confirm(`Confirma o cadastro da nave ${name} com piloto ${pilot} e limite de tripulantes ${crewLimit}?`);
    if (confirmation) {
        const newSpaceship = addSpaceship(name, pilot, crewLimit);
        alert(`Nave ${newSpaceship.name} cadastrada com sucesso! Piloto: ${newSpaceship.pilot}, Limite de tripulantes: ${newSpaceship.crewLimit}`);
    }
    else {
